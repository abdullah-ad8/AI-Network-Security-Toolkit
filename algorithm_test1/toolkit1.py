...
class NetworkScanner:
    ...
    def discover_devices(self):
        print(f"[*] Scanning network: {self.target_network} for active devices...")
        return ["Attacker_PC", "HR_Laptop", "Dev_PC", "Main_Server"]
...
def main():
    ...
    attack_graph = mapper.build_graph(simulated_network)
    
    analyzer = PathAnalyzer(attack_graph)
    best_path, path_cost = analyzer.find_shortest_attack_path("Attacker_PC", "Main_Server")

    # تشغيل تحليل BFS الجديد
    connectivity_results = analyzer.analyze_connectivity_bfs("Attacker_PC")
    ...
    if best_path:
        ...
        report = ai_module.generate_report(best_path, path_cost, simulated_network)
        
        print("================ AI SECURITY REPORT ================")
        print(report)
        # استدعاء دالة الرسم وتمرير أفضل مسار لها ليتم تلوينه
